...
public class PlayerFish extends Entity implements PlayerFishInterface {
  ...
  public PlayerFish(int movespeed, boolean isAlive, Sprite sprite, int score) {
    ...
    if (sprite.getImg() != null) {
      ...
      setPlayerFishLeftImage(new Image(leftImageName, imgWidth, imgHeight, true, true));
      setPlayerFishRightImage(new Image(rightImageName, imgWidth, imgHeight, true, true));
    } else {
   ...
   * The image is scaled to its starting size.
   * A BoundingBox with the same dimensions is created and placed at the middle of the screen.
   * 
  ...
  public static PlayerFish createPlayerFish() {
    ...
    Image playerFishImage = new Image(leftImageName, startImageWidth, startImageHeight, true, true);

    int startPosX = MainScreenController.getScreenbox().getWidth() / 2;
    int startPosY = MainScreenController.getScreenbox().getHeight() / 2;
    
    BoundingBox boundingBox = new BoundingBox(startPosX, startPosY, startImageWidth, startImageHeight);
  ...
  /**
   * {@inheritDoc} Check if the x-coordinate is <= 0;
   */
  ...
  /**
   * {@inheritDoc} Check if the x-coordinate + width >= x-resolution.
   */
  ...
  /**
   * {@inheritDoc} Check if the y-coordinate <= 0;
   */
  ...
  /**
   * {@inheritDoc} Check if the y-coordinate + height => y-resolution.
   */
